...
fn test_config() -> LocalComputeConfig {
    LocalComputeConfig {
        supervisor_bin: PathBuf::from("/usr/bin/openshell-sandbox"),
        grpc_endpoint: "http://localhost:17670".to_string(),
...
fn test_config_validation_empty_binary() {
    ...
    config.supervisor_bin = PathBuf::new();
    let rt = tokio::runtime::Runtime::new().unwrap();
    let result = rt.block_on(LocalComputeDriver::new(
        config,
        Arc::new(MockSupervisorReadiness::new()),
    ));
    assert!(result.is_err());
    assert!(result.unwrap_err().contains("supervisor binary path is required"));
}

#[test]
fn test_config_validation_missing_binary() {
    let mut config = test_config();
    config.supervisor_bin = PathBuf::from("/nonexistent/path/to/sandbox");
    let rt = tokio::runtime::Runtime::new().unwrap();
    let result = rt.block_on(LocalComputeDriver::new(
        config,
        Arc::new(MockSupervisorReadiness::new()),
...
fn test_config_validation_empty_endpoint() {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().contains("gRPC endpoint is required"));
}
...
fn test_build_snapshot_not_running() {
    ...
    let snapshot = LocalComputeDriver::build_snapshot(&state, false);
...
fn test_build_snapshot_running_and_connected() {
    ...
    let snapshot = LocalComputeDriver::build_snapshot(&state, true);
    let status = snapshot.status.unwrap();
...
fn test_capabilities() {
    ...
    assert_eq!(caps.driver_name, "local");
